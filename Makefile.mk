...
runner: requirexcbeautify requiretuist
	...
	set -o pipefail && xcodebuild build -scheme "${TARGET_NAME}" -destination "OS=18.2,name=iPhone 16 Pro" -skipPackagePluginValidation | xcbeautify
	set -o pipefail && xcodebuild build -scheme "${TARGET_NAME}" -destination "platform=macOS,arch=arm64" -skipPackagePluginValidation | xcbeautify
